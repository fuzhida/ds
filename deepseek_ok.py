...
def get_btc_ohlcv():
    ...
    try:
        # 获取最近96根K线
        ohlcv = exchange.fetch_ohlcv(TRADE_CONFIG['symbol'], TRADE_CONFIG['timeframe'], limit=96)
...
def analyze_with_deepseek(price_data):
    ...
    try:
        response = deepseek_client.chat.completions.create(
            ...
            messages=[
                {"role": "system",
                 "content": f"您是一位急需资金为母亲治疗癌症的smc/ict日内交易员，擅长在周开盘价、7日平均振幅、前一日高低点、日开盘价、4小时关键位置等日内关键偏好位置做单，你现在需要从加密货币交易市场赚取足够的金额治疗你的母亲，专注于{TRADE_CONFIG['timeframe']}周期趋势分析。请结合K线形态和技术指标做出判断。"},

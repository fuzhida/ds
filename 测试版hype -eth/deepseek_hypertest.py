...
# Import CoinDesk WebSocket indicators provider as primary data source
from coindesk_websocket_indicators import CoinDeskWebSocketIndicatorProvider, CoinDeskIndicatorConfig
# Import Hyperliquid WebSocket as backup data source
...
# Detailed config info to DEBUG level
system_logger.debug("Detailed config - Amount=%.4f BTC, Timeframes=%s/%s, Threshold=%.3f%%, Margin=%.1f%%, Min Position=%d USDC", 
...
system_logger.debug("Risk controls - R:R thresholds=%.1f:1/%.1f:1, Aggressive risk=%.1f%%, Daily loss=%.1f%%, Drawdown=%.1f%%",
...
# New: Log new configs
system_logger.debug("Multi-timeframe config - Higher TF bias=%s, Lower TF entry=%s", config.higher_tf_bias_tf, config.lower_tf_entry_tf)
system_logger.debug("Confirmation signals - Volume threshold=%.1fx MA, FVG stacking=%d, Candle pattern weight=%.1fx, Fresh zone max interactions=%d",
...
class TradingBot:
    def __init__(self, config: Config, exchange=None):
        ...
        # Initialize CoinDesk WebSocket (primary) and Hyperliquid backup providers
        self.coindesk_provider = None
        self.hyperliquid_websocket_backup = None
        self.hyperliquid_backup = None
        
        # Initialize CoinDesk WebSocket indicator provider (primary)
        try:
            # 直接从文件读取API密钥
            ...
            try:
                with open('1.env', 'r') as f:
                    for line in f:
                        if line.startswith('COINDESK_API_KEY='):
                            coindesk_api_key = line.split('=', 1)[1].strip()
                            break
            except Exception as e:
                self.logger_system.error(f"读取1.env文件失败: {e}")
            
            if not coindesk_api_key:
                coindesk_api_key = os.getenv('COINDESK_API_KEY')
            
            self.logger_system.info(f"主程序读取到的COINDESK_API_KEY: {coindesk_api_key[:10] if coindesk_api_key else 'None'}...")
            
            coindesk_config = CoinDeskIndicatorConfig(
                symbol=self.config.symbol.replace('/USDC:USDC', '').replace('/', ''),  # Convert ETH/USDC:USDC to ETH
                timeframes=['1m', '5m', '15m', '1h', '4h', '1d'],
                indicators=['rsi', 'macd', 'ema', 'sma', 'atr', 'bb'],
                api_key=coindesk_api_key  # 直接传递API密钥
            )
            self.coindesk_provider = CoinDeskWebSocketIndicatorProvider(coindesk_config)
            self.coindesk_provider.start()  # 启动WebSocket连接
            self.logger_system.info(f"✓ CoinDesk WebSocket indicator provider initialized for {coindesk_config.symbol}")
        except Exception as e:
            self.logger_system.error(f"CoinDesk WebSocket indicator provider initialization failed: {e}")
        
        # Initialize Hyperliquid WebSocket backup provider
        try:
            hyperliquid_config = HyperliquidIndicatorConfig(
                symbol=self.config.symbol.replace('/USDC:USDC', '').replace('/', ''),  # Convert ETH/USDC:USDC to ETH
                timeframes=['1m', '5m', '15m', '1h', '4h', '1d'],
                indicators=['rsi', 'macd', 'ema', 'sma', 'atr', 'bb']
            )
            self.hyperliquid_websocket_backup = HyperliquidWebSocketProvider(hyperliquid_config)
            self.logger_system.info(f"✓ Hyperliquid WebSocket backup provider initialized for {hyperliquid_config.symbol}")
        except Exception as e:
            self.logger_system.error(f"Hyperliquid WebSocket backup provider initialization failed: {e}")
        
        # Initialize Hyperliquid API backup provider
        try:
            self.hyperliquid_backup = HyperliquidBackupProvider(self.exchange)
            self.logger_system.info("✓ Hyperliquid API backup provider initialized")
        except Exception as e:
            self.logger_system.error(f"Hyperliquid API backup provider initialization failed: {e}")
        
        # Data source priority explanation
        if self.coindesk_provider and self.hyperliquid_websocket_backup and self.hyperliquid_backup:
            self.logger_system.info("Data source priority: CoinDesk WebSocket → Hyperliquid WebSocket → Hyperliquid API")
        elif self.coindesk_provider and self.hyperliquid_backup:
            self.logger_system.info("Data source priority: CoinDesk WebSocket → Hyperliquid API")
        elif self.coindesk_provider:
            self.logger_system.info("Data source: WebSocket indicators (only available)")
        elif self.hyperliquid_backup:
            self.logger_system.info("Data source: Hyperliquid backup API (only available)")
        else:
            self.logger_system.warning("No data sources available!")
    ...
    def safe_fetch_ticker(self, exchange, symbol):
        """
        Fetch real-time price data by priority:
        1. WebSocket real-time price (if available)
        2. Hyperliquid API (backup)
        Disable simulated data, only use real market data
        """
        
        # Priority 1: Try CoinDesk WebSocket indicator provider for real-time price
        if self.coindesk_provider:
            try:
                self.logger_api.debug(f"Attempting to fetch {symbol} real-time price from CoinDesk WebSocket")
                # Convert symbol format for WebSocket (ETH/USDC:USDC -> ETH)
                ws_symbol = symbol.replace('/USDC:USDC', '').replace('/', '')
                current_price = self.coindesk_provider.get_current_price(ws_symbol)
                if current_price and current_price > 0:
                    self.logger_api.debug(f"✓ CoinDesk WebSocket fetched {symbol} real-time price: ${current_price}")
                    return {
                        'symbol': symbol,
                        'last': current_price,
                        'timestamp': int(time.time() * 1000)
                ...
                self.logger_api.warning(f"CoinDesk WebSocket real-time price fetch failed, trying next data source: {e}")
        
        # Priority 2: Try Hyperliquid WebSocket backup provider for real-time price
        if self.hyperliquid_websocket_backup:
            try:
                self.logger_api.debug(f"Attempting to fetch {symbol} real-time price from Hyperliquid WebSocket backup")
                # Convert symbol format for WebSocket (ETH/USDC:USDC -> ETH)
                ws_symbol = symbol.replace('/USDC:USDC', '').replace('/', '')
                current_price = self.hyperliquid_websocket_backup.get_current_price(ws_symbol)
                if current_price and current_price > 0:
                    self.logger_api.debug(f"✓ Hyperliquid WebSocket backup fetched {symbol} real-time price: ${current_price}")
                    return {
                        'symbol': symbol,
                        'last': current_price,
                        'timestamp': int(time.time() * 1000)
                ...
                self.logger_api.warning(f"Hyperliquid WebSocket backup real-time price fetch failed, trying next data source: {e}")
        
        # Priority 2: Try Hyperliquid backup provider
        if self.hyperliquid_backup:
            try:
                self.logger_api.debug(f"Attempting to fetch {symbol} real-time price from Hyperliquid backup")
                current_price = self.hyperliquid_backup.get_current_price(symbol)
                if current_price and current_price > 0:
                    self.logger_api.debug(f"✓ Hyperliquid backup fetched {symbol} real-time price: ${current_price}")
                    return {
                        'symbol': symbol,
                        'last': current_price,
                        'timestamp': int(time.time() * 1000)
                ...
                self.logger_api.warning(f"Hyperliquid backup real-time price fetch failed, trying next data source: {e}")
        
        # Priority 3: Last backup - Hyperliquid API (only if exchange not None)
        if exchange is not None:
            try:
                self.logger_api.debug(f"Attempting to fetch {symbol} real-time price from Hyperliquid API")
                ticker = exchange.fetch_ticker(symbol)
                if ticker and ticker.get('last') and ticker['last'] > 0:
                    self.logger_api.debug(f"✓ Hyperliquid API fetched {symbol} real-time price: ${ticker['last']}")
                    return ticker
            except Exception as e:
                self.logger_api.error(f"Hyperliquid API real-time price fetch failed: {e}")
        else:
            self.logger_api.warning("Hyperliquid exchange not initialized, skipping Hyperliquid API real-time price fetch")
        
        # If all data sources fail, return None instead of raising exception
        error_msg = f"All data sources failed to fetch {symbol} real-time price, prohibit using fake data"
        self.logger_api.error(error_msg)
        return None

    @retry_on_exception(retries=3)
    def safe_fetch_ohlcv(self, exchange, symbol, timeframe, limit, since=None):
        """
        Fetch OHLCV data by priority:
        1. WebSocket indicators provider (real-time data)
        2. Hyperliquid backup provider (backup signal source)
        3. Hyperliquid API (last backup)
        Disable simulated data, only use real market data
        """
        
        # Priority 1: Try CoinDesk WebSocket indicators provider for real-time data
        if hasattr(self, 'coindesk_provider') and self.coindesk_provider:
            try:
                # Convert symbol format for WebSocket (ETH/USDC:USDC -> ETHUSDC)
                ws_symbol = symbol.replace('/', '').replace(':USDC', '')
                self.logger_api.debug(f"Attempting to fetch {ws_symbol} {timeframe} data from CoinDesk WebSocket")
                ohlcv_data = self.coindesk_provider.get_ohlcv_data(ws_symbol, timeframe, limit, since)
                if ohlcv_data and len(ohlcv_data) > 0:
                    self.logger_api.info(f"✓ CoinDesk WebSocket fetched {len(ohlcv_data)} {timeframe} data points")
                    return ohlcv_data
            except Exception as e:
                self.logger_api.warning(f"CoinDesk WebSocket data fetch failed, trying next data source: {e}")
        
        # Priority 2: Try Hyperliquid WebSocket backup provider for real-time data
        if hasattr(self, 'hyperliquid_websocket_backup') and self.hyperliquid_websocket_backup:
            try:
                # Convert symbol format for WebSocket (ETH/USDC:USDC -> ETHUSDC)
                ws_symbol = symbol.replace('/', '').replace(':USDC', '')
                self.logger_api.debug(f"Attempting to fetch {ws_symbol} {timeframe} data from Hyperliquid WebSocket backup")
                ohlcv_data = self.hyperliquid_websocket_backup.get_ohlcv_data(ws_symbol, timeframe, limit, since)
                if ohlcv_data and len(ohlcv_data) > 0:
                    self.logger_api.info(f"✓ Hyperliquid WebSocket backup fetched {len(ohlcv_data)} {timeframe} data points")
                    return ohlcv_data
            except Exception as e:
                self.logger_api.warning(f"Hyperliquid WebSocket backup data fetch failed, trying next data source: {e}")
        
        # Priority 2: Try Hyperliquid backup provider
        if hasattr(self, 'hyperliquid_backup') and self.hyperliquid_backup:
            try:
                self.logger_api.debug(f"Attempting to fetch {symbol} {timeframe} data from Hyperliquid backup provider")
                ohlcv_data = self.hyperliquid_backup.get_ohlcv_data(symbol, timeframe, limit, since)
                if ohlcv_data and len(ohlcv_data) > 0:
                    self.logger_api.info(f"✓ Hyperliquid backup provider fetched {len(ohlcv_data)} {timeframe} data points")
                    return ohlcv_data
            except Exception as e:
                self.logger_api.warning(f"Hyperliquid backup provider data fetch failed, trying next data source: {e}")
        
        # Priority 3: Last backup - Hyperliquid API (only if exchange not None)
        if exchange is not None:
            try:
                self.logger_api.debug(f"Attempting to fetch {symbol} {timeframe} data from Hyperliquid API")
                ohlcv_data = exchange.fetch_ohlcv(symbol, timeframe, since, limit)
                if ohlcv_data and len(ohlcv_data) > 0:
                    self.logger_api.info(f"✓ Hyperliquid API fetched {len(ohlcv_data)} {timeframe} data points")
                    return ohlcv_data
            except Exception as e:
                self.logger_api.error(f"Hyperliquid API data fetch failed: {e}")
        else:
            self.logger_api.warning("Hyperliquid exchange not initialized, skipping Hyperliquid API data fetch")
        
        # If all data sources fail, return None instead of raising exception
        error_msg = f"All data sources failed to fetch {symbol} {timeframe} data, prohibit using fake data"
        self.logger_api.error(error_msg)
        return None

    def _generate_simulated_ohlcv(self, limit):
        """
        Disabled: No longer generate simulated OHLCV data
        Only use real market data, prohibit fake data
        """
        error_msg = "Simulated data generation disabled, only allow real market data"
        self.logger_api.error(error_msg)
    ...
    def safe_fetch_positions(self, exchange, symbols):
        ...
        # For Hyperliquid, add user parameter if wallet address available
        wallet_address = os.getenv('HYPERLIQUID_WALLET_ADDRESS')
        if wallet_address:
    ...
    def safe_fetch_order(self, exchange, order_id, symbol):
        if self.config.simulation_mode:
            # Simulation mode returns simulated order status
            return {
                'id': order_id,
                'symbol': symbol,
                'status': 'closed',
    ...
    def safe_set_leverage(self, exchange, leverage, symbol, params):
        """Set leverage and verify result"""
        if self.config.simulation_mode:
            self.logger_trading.info(f"Simulation mode: Set leverage {leverage}x for {symbol}")
            return {'leverage': leverage, 'symbol': symbol, 'simulated': True}
        
        try:
            # Set leverage (Hyperliquid specific: no mgnMode or posSide)
            result = exchange.set_leverage(leverage, symbol, params)
            self.logger_trading.info(f"Leverage set result: {result}")
            
            # Verify leverage set successfully
            ...
            if actual_leverage is not None and abs(actual_leverage - leverage) < 0.1:
                self.logger_trading.info(f"Leverage verification successful: Expected {leverage}x, actual {actual_leverage}x")
                return result
            elif actual_leverage is None:
                self.logger_trading.warning(f"Leverage verification failed: Unable to fetch current leverage setting (possibly network issue)")
                # If unable to fetch leverage info, assume set successfully
                self.logger_trading.info(f"Assume leverage set successful: {leverage}x (due to network issue unable to verify)")
                return result
            else:
                self.logger_trading.warning(f"Leverage verification failed: Expected {leverage}x, actual {actual_leverage}x")
                # If verification fails, try setting again
                result = exchange.set_leverage(leverage, symbol, params)
                self.logger_trading.info(f"Re-set leverage: {result}")
                return result
            
        except Exception as e:
            self.logger_trading.error(f"Leverage setting failed: {e}")
            raise

    @retry_on_exception(retries=3)
    def safe_fetch_leverage(self, exchange, symbol):
        """Fetch current leverage multiplier"""
        if self.config.simulation_mode:
            # Simulation mode returns configured leverage
            return self.config.leverage
            
        try:
            # Add user parameter for Hyperliquid API
            wallet_address = os.getenv('HYPERLIQUID_WALLET_ADDRESS')
                ...
                for pos in positions:
                    if pos['symbol'] == symbol:
                        ...
                        if leverage is not None:
                            self.logger_trading.debug(f"Fetched leverage: {leverage}x")
                            return leverage
                        else:
                            self.logger_trading.debug(f"No leverage info in position: {pos}")
                            return None
            self.logger_trading.debug(f"No position info found for {symbol}")
            return None
        except Exception as e:
            self.logger_trading.warning(f"Fetch leverage failed: {e}")
            return None

    @retry_on_exception(retries=3)
    def safe_fetch_open_orders(self, exchange, symbol, params=None):
        if self.config.simulation_mode:
            # Simulation mode returns empty order list
            return []
    ...
    def _perform_initial_api_health_check(self):
        """Perform initial API health check"""
        if self.config.simulation_mode:
            self.logger_api.info("Simulation mode - Skip initial API health check")
            return
        ...
        try:
            ...
            if response and response.choices:
                ...
                if previous_status != 'healthy':
                    self.logger_api.info("DeepSeek API recovered (consecutive failures: %d)", 
                                       self.api_health_status['deepseek'].get('consecutive_failures', 0))
                else:
                    # Sample log: Log every 10 checks
                    self.log_counters['api_health_check'] += 1
                    if self.log_counters['api_health_check'] % 10 == 0:
                        self.logger_api.debug("DeepSeek API health check: Healthy (check #%d)", self.log_counters['api_health_check'])
            ...
            # Adjust log level based on failure count
            if failures == 1:
                self.logger_api.warning("DeepSeek API health check failed: %s", e)
            elif failures <= 3:
                self.logger_api.warning("DeepSeek API consecutive failures %d: %s", failures, e)
            else:
                self.logger_api.error("DeepSeek API severe failure (consecutive failures %d): %s", failures, e)
    ...
    def _check_hyperliquid_health(self) -> bool:
        ...
        if self.config.simulation_mode:
            # Skip real API health check in simulation mode
            current_time = time.time()
            ...
            # Simulation mode log only on first or status change
            if not hasattr(self, '_hyperliquid_sim_logged'):
                self.logger_api.info("Hyperliquid API health check: Simulation mode - Skip")
        ...
        try:
            ...
            if ticker and 'last' in ticker:
                ...
                # Sample log: Log every 10 successful checks, or on recovery from unhealthy
                ...
                if self.log_counters['hyperliquid_health'] % 10 == 0:
                    self.logger_api.debug("Hyperliquid API health check: Healthy (cumulative success: %d)", self.log_counters['hyperliquid_health'])
                elif was_unhealthy:
                    self.logger_api.info("Hyperliquid API health check: Recovered")
            ...
            # Adjust log level based on consecutive failure count
            if failures == 1:
                self.logger_api.warning("Hyperliquid API health check failed: %s", e)
            elif failures <= 3:
                self.logger_api.warning("Hyperliquid API persistent failure: %s (consecutive failures: %d)", e, failures)
            else:
                self.logger_api.error("Hyperliquid API severe failure: %s (consecutive failures: %d)", e, failures)
            
            return False

    def _should_check_api_health(self, api_name: str) -> bool:
        """Determine if API health check needed"""
        current_time = time.time()
    ...
    def _update_api_failure(self, api_name: str, error: Exception):
        ...
        if failures >= 3:
            self.logger_api.error(f"{api_name} API consecutive failures {failures}, possible serious issue: {error}")
        elif failures >= 5:
            self.logger_api.critical(f"{api_name} API consecutive failures {failures}, suggest checking network and API config")
            
    def _update_api_success(self, api_name: str):
        """Update API success status"""
    ...
    def _reset_daily_balance(self):
        ...
        if today != self.last_reset_date:
            ...
            self.logger_system.info(f"Daily balance reset: {self.daily_start_balance:.2f} USDT")
        ...
        if (datetime.now(timezone.utc) - self.last_reset_4h).total_seconds() > 14400:  # 4h
            ...
            self.logger_system.info(f"4h balance reset: {self.daily_start_balance:.2f} USDT")
    ...
    def _check_risk_controls(self, proposed_risk: float) -> bool:
        ...
        if daily_loss > self.daily_start_balance * self.config.max_daily_loss_pct:
            self.logger_risk.error("Exceeded daily loss limit: Loss=%.2f USDC (%.1f%%) > Limit=%.2f USDC (%.1f%%), Balance=%.2f", 
        ...
        if drawdown > self.config.max_drawdown_pct and drawdown > 0.01:  # Ignore small drawdown
            self.logger_risk.error("Exceeded max drawdown: Drawdown=%.1f%% > Limit=%.1f%%, Peak=%.2f, Current=%.2f", 
        ...
        if open_pos >= self.config.max_open_positions:
            self.logger_risk.warning("Reached max positions: Current=%d >= Limit=%d, skip new trade", 
        ...
        if proposed_risk > max_risk:
            self.logger_risk.warning("Proposed risk exceeds limit: Risk=%.2f USDC > Limit=%.2f USDC (%.1f%%), Balance=%.2f", 
                                   proposed_risk, max_risk, self.config.risk_per_trade*100, self.current_balance)
            return False

        # Log risk check passed (sample log)
        self.log_counters['risk_check'] = self.log_counters.get('risk_check', 0) + 1
        if self.log_counters['risk_check'] % 10 == 1:  # Log every 10 checks
            self.logger_risk.debug("Risk check passed: Proposed risk=%.2f, Daily loss=%.1f%%, Drawdown=%.1f%%, Positions=%d/%d", 
    ...
    def _update_current_balance(self):
        try:
            ...
            self.logger_system.warning(f"Update balance failed: {e}")
        ...
        with self.lock:
            if os.path.exists(self.config.signals_file):
                try:
                    ...
                    self.logger_system.exception(f"Load signal history failed: {e}")
                    self.signal_history = []
            else:
                self.signal_history = []
    ...
    def save_signal_history(self):
        try:
            # Copy signal history under lock to avoid modification during write
            with self.lock:
            ...
            self.logger_system.exception(f"Save signal history failed: {e}")

    def heartbeat(self):
        try:
            with self.lock:
                current_pos = self.get_current_position()
                pos_str = f"Position: {current_pos['side']} {current_pos['size']:.4f} @ {current_pos['entry_price']:.2f} (SL: {current_pos.get('liquidation_price', 'N/A'):.2f})" if current_pos else "No position"
                
            if self.config.simulation_mode:
                # Use simulated data in simulation mode
                balance_free = self.initial_balance
                bal_str = f"Balance: {balance_free:.2f} USD (simulated)"
                ...
                bal_str = f"Balance: {balance_free:.2f} USD"
                ticker = self.safe_fetch_ticker(exchange, self.config.symbol)
                current_price = round(ticker['last'], 2) if ticker else 67000.0  # FIXED: Medium 3 - Round
                
            with self.lock:
                sig_count = len(self.signal_history)
                last_sig = self.signal_history[-1]['signal'] if self.signal_history else 'None'
            ...
            if balance_free < self.initial_balance * 0.9:
                has_anomaly = True
                anomaly_messages.append(f" Balance low: {bal_str} (below initial balance 90%)")
            
            # Check position time too long
            if current_pos and current_pos.get('entry_time'):
                ...
                if position_duration > self.config.max_position_time:
                    ...
                    anomaly_messages.append(f" Position time too long: {hours:.1f} hours (exceeds {self.config.max_position_time/3600:.1f} hour limit)")
            ...
            if daily_loss_pct > self.config.max_daily_loss_pct * 100:
                has_anomaly = True
                anomaly_messages.append(f" Daily loss exceeds limit: {daily_loss_pct:.1f}%")
            ...
            if drawdown_pct > self.config.max_drawdown_pct * 100:
                has_anomaly = True
                anomaly_messages.append(f" Drawdown exceeds limit: {drawdown_pct:.1f}%")
            
            # Check API health status
            for api_name, health_info in self.api_health_status.items():
                if health_info['consecutive_failures'] >= 3:
                    has_anomaly = True
                    anomaly_messages.append(f" {api_name} API consecutive failures: {health_info['consecutive_failures']} times")
            
            # Compress heartbeat log to one line, detail only on anomaly
            anomaly_msg = "; ".join(anomaly_messages) if has_anomaly else "Normal"
            self.logger_monitor.info("Heartbeat: %s | Position=%s | Balance=%.2f USD | Signal count=%d | Price=%.2f%s", 
                                   datetime.now(timezone.utc).strftime('%H:%M:%S'), 
                                   pos_str, balance_free, sig_count, current_price, 
                                   f" (Anomaly: {anomaly_msg})" if has_anomaly else "")
            
            # Extra detail on anomaly
            if has_anomaly:
                self.logger_monitor.warning("Heartbeat anomaly details: %s", "; ".join(anomaly_messages))
            
            # Always write heartbeat file for monitoring
            ...
            self.logger_monitor.error("Heartbeat detection failed: %s", e)

    def setup_exchange(self):
        try:
            if self.config.simulation_mode:
                self.logger_system.info("Simulation mode: Skip exchange API connection")
                self.initial_balance = 10000.0  # Simulated balance
                self.logger_system.info(f"Simulated USD balance: {self.initial_balance:.2f}")
            else:
                ...
                self.logger_trading.info(f"Set leverage: {target_leverage}x")
                balance = self.safe_fetch_balance(exchange)
                ...
                self.last_reset_4h = datetime.now(timezone.utc)
                self.logger_system.info(f"Current USD balance: {usd_balance:.2f}")
                self.logger_system.info(f"Initial balance set to: {self.initial_balance:.2f}")
                self.logger_trading.info(f"Adaptive leverage: {self.config.leverage}x (max for {self.config.symbol}: {max_l}x)")
            ...
            if self.position_store.get():
                ...
                if self.config.simulation_mode:
                    ...
                    self.logger_system.info("Simulation mode - Set current time as entry time")
                else:
                    try:
                        ...
                        if trades and len(trades) > 0:
                            ...
                            self.logger_trading.info(f"Restored entry time from trade history: {current_pos['entry_time']}")
                        else:
                            current_pos['entry_time'] = None  # No trade history, mark as new or empty
                            self.logger_trading.warning("No recent trade history found, entry time set to None")
                    except Exception as e:
                        self.logger_trading.warning(f"Restore entry time from trade history failed: {e}, use None")
                        current_pos['entry_time'] = None
                self.position_store.set(current_pos)
                self.logger_trading.info("Detected existing position, set entry time")
            ...
            self.logger_system.debug("Initial key levels cache set (empty data fallback)")

            return True
        except Exception as e:
            self.logger_system.exception(f"Exchange setup failed: {e}")
            return False
    ...
    def check_balance_risk(self):
        try:
            ...
            if current_balance < min_required:
                ...
                self.logger_risk.warning("Balance risk triggered, stop trading: Current=%.2f USDC < Threshold=%.2f USDC (%.1f%% < %.1f%%), Loss=%.1f%%", 
                                       current_balance, min_required, 
                                       balance_ratio*100, self.config.min_balance_ratio*100, loss_pct)
                return False
            
            # Log normal status every 50 checks (sample)
            self.log_counters['balance_check'] = self.log_counters.get('balance_check', 0) + 1
            if self.log_counters['balance_check'] % 50 == 0:
                self.logger_risk.debug("Balance check normal: Current=%.2f USDC, Ratio=%.1f%%, Initial=%.2f USDC (check #%d)", 
                                     current_balance, balance_ratio*100, self.initial_balance, 
                                     self.log_counters['balance_check'])
            return True
        except Exception as e:
            self.logger_risk.error("Balance check failed: %s", str(e))
            self.logger_risk.exception("Balance check detailed error")
            return False
    ...
    def check_position_time(self):
        ...
        if position_duration > max_duration:
            ...
            self.logger_risk.warning("Position time exceeds limit, trigger forced close: Duration=%.1f hours > Limit=%.1f hours, Side=%s, Size=%.4f, Unrealized PnL=%.2f USDC", 
    ...
    def close_position(self):
        ...
        try:
            ...
            self.logger_trading.info("Starting forced close: ID=%s, Side=%s→%s, Size=%.4f, Entry=%.2f, Unrealized PnL=%.2f USDC", 
            ...
            self.logger_trading.info("Forced close successful: ID=%s, Order ID=%s, Filled=%.4f", 
            ...
            self.logger_trading.error("Forced close failed: ID=%s, Error=%s, Position details=%s", 
                                    close_id, str(e), close_context)
            self.logger_trading.exception("Forced close detailed error: ID=%s", close_id)
    ...
    def calculate_technical_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        try:
            ...
            # Check data length, ensure enough data for all indicators - Optimized for live trading
            min_required = 20  # Further reduced to 20 data points, suitable for live trading
            absolute_min = 5  # Reduced absolute min data requirement to 5
            
            if len(df) < min_required:
                self.logger_system.warning(f"Data length insufficient ({len(df)} < {min_required}), attempting fallback calculation")
            ...
            # Exponential moving averages (for trend judgment and stop profit/loss)
            ...
            # Short and long EMA for signal confirmation
            df['ema_short'] = df['ema_12']  # Short EMA
            df['ema_long'] = df['ema_26']   # Long EMA
            ...
            # Bollinger Bands indicator (for support/resistance judgment)
            ...
            # Support/resistance levels (for stop profit/loss optimization)
            df['resistance'] = df['high'].rolling(20, min_periods=1).max()
            df['support'] = df['low'].rolling(20, min_periods=1).min()
            df['pivot'] = (df['high'] + df['low'] + df['close']) / 3
            
            # Volume indicator (for signal strength confirmation)
            df['volume_ma'] = df['volume'].rolling(20, min_periods=1).mean()
            df['volume_ratio'] = df['volume'] / df['volume_ma']
            
            # Price position indicator (for stop profit/loss adjustment)
            df['price_position'] = (df['close'] - df['support']) / (df['resistance'] - df['support'])
            
            # Volatility indicator (for dynamic stop loss distance adjustment)
            df['volatility'] = df['close'].rolling(14, min_periods=1).std() / df['close'].rolling(14, min_periods=1).mean()
            
            # Trend strength indicator
            df['trend_strength'] = abs(df['ema_12'] - df['ema_26']) / df['atr']
            ...
            for col in ['rsi', 'macd', 'atr', 'sma_20', 'sma_50', 'bb_position', 'price_position', 'volume_ratio']:
                if col in df.columns:
                    if col in ['bb_position', 'price_position']:
                        df[col] = df[col].clip(0, 1)  # Position indicators limited to 0-1
                    elif col == 'volume_ratio':
                        df[col] = df[col].clip(0, 10)  # Volume ratio limited to reasonable range
                    else:
            ...
            self.logger_system.info(f"Technical indicators calculation completed, contains {len(df)} data points")
            return df
        except Exception as e:
            self.logger_system.exception(f"Technical indicators calculation failed: {e}")
            return df

    def _calculate_fallback_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        """Calculate basic technical indicators when data insufficient"""
        try:
            self.logger_system.info(f"Using {len(df)} data points for fallback indicators")
            ...
            # Basic moving averages (use smaller window, min 2 data points)
            ...
            # Exponential moving averages (min 2 data points)
            ...
            # RSI (use smaller period, min 2 data points)
            ...
            # ATR (use smaller period, min 2 data points)
            atr_period = max(2, min(14, data_len))
            if data_len >= 2:
                high_low = df['high'] - df['low']
                high_close = np.abs(df['high'] - df['close'].shift())
                low_close = np.abs(df['low'] - df['close'].shift())
            ...
            # Fill missing values - FIXED: High 7 - Unified
            ...
            self.logger_system.info("Fallback indicators calculation successful")
            return df
            
        except Exception as e:
            self.logger_system.exception(f"Fallback indicators calculation failed: {e}")
            return df

    def calculate_fibonacci_levels(self, df: pd.DataFrame, lookback: int = 20) -> Dict[str, float]:
        """
        Calculate Fibonacci retracement and extension levels
        For optimizing stop profit/loss positions
        """
        try:
            if len(df) < lookback:
                lookback = len(df)
            ...
            # Fibonacci retracement levels (from high retracement)
            fib_retracements = {
            ...
            # Fibonacci extension levels (targets after breakout)
            fib_extensions = {
            ...
            self.logger_system.info(f"Fibonacci levels calculation completed: High={high:.2f}, Low={low:.2f}, Range={price_range:.2f}")
            return fib_levels
            
        except Exception as e:
            self.logger_system.error(f"Fibonacci levels calculation failed: {e}")
            return {}

    def get_optimal_fibonacci_sl_tp(self, df: pd.DataFrame, signal: str, current_price: float) -> Tuple[float, float]:
        """
        Calculate optimal stop loss/take profit based on Fibonacci levels
        """
        try:
            ...
            if signal == 'BUY':
                # Long trade: Find support below as stop loss, resistance above as take profit
                potential_sl = []
                potential_tp = []
                
                # Stop loss candidates: Fibonacci levels below current price
                for level_name, level_price in fib_levels.items():
                    if 'fib_' in level_name and level_price < current_price:
                        distance = current_price - level_price
                        if 0.5 * atr <= distance <= 3 * atr:  # Reasonable stop loss distance
                            potential_sl.append((level_name, level_price, distance))
                
                # Take profit candidates: Fibonacci levels above current price
                for level_name, level_price in fib_levels.items():
                    if ('fib_ext_' in level_name or level_name == 'swing_high') and level_price > current_price:
                        distance = level_price - current_price
                        if 1 * atr <= distance <= 5 * atr:  # Reasonable take profit distance
                            potential_tp.append((level_name, level_price, distance))
                
                # Select optimal stop loss (nearest reasonable support)
                if potential_sl:
                    potential_sl.sort(key=lambda x: x[2])  # Sort by distance
                    stop_loss = potential_sl[0][1]
                    self.logger_trading.info(f"Selected Fibonacci stop loss: {potential_sl[0][0]} = {stop_loss:.2f}")
                else:
                    stop_loss = current_price - atr * 2  # Fallback to ATR stop loss
                
                # Select optimal take profit (reasonable resistance, prioritize 1.618 extension)
                if potential_tp:
                    # Prioritize 1.618 extension level
                    fib_618_tp = [tp for tp in potential_tp if 'fib_ext_161.8' in tp[0]]
                    if fib_618_tp:
                        take_profit = fib_618_tp[0][1]
                        self.logger_trading.info(f"Selected Fibonacci take profit: {fib_618_tp[0][0]} = {take_profit:.2f}")
                    else:
                        potential_tp.sort(key=lambda x: x[2])  # Sort by distance, select nearest
                        take_profit = potential_tp[0][1]
                        self.logger_trading.info(f"Selected Fibonacci take profit: {potential_tp[0][0]} = {take_profit:.2f}")
                else:
                    take_profit = current_price + atr * 3  # Fallback to ATR take profit
                    
            else:  # SELL
                # Short trade: Find resistance above as stop loss, support below as take profit
                potential_sl = []
                potential_tp = []
                
                # Stop loss candidates: Fibonacci levels above current price
                for level_name, level_price in fib_levels.items():
                    if 'fib_' in level_name and level_price > current_price:
                        distance = level_price - current_price
                ...
                # Take profit candidates: Fibonacci levels below current price
                for level_name, level_price in fib_levels.items():
                    if 'fib_' in level_name and level_price < current_price:
                        distance = current_price - level_price
                ...
                # Select optimal stop loss (nearest reasonable resistance)
                if potential_sl:
                ...
                # Select optimal take profit (reasonable support)
                if potential_tp:
                    # Prioritize 61.8% retracement level
            ...
            # New: OTE retracement filter - Entry buffer at 0.618 Fib
            if signal == 'BUY':
                ote_level = fib_levels.get('fib_61.8', current_price * 0.618)
                if current_price < ote_level:
                    stop_loss = max(stop_loss, ote_level)  # Buffer SL
            else:
                ote_level = fib_levels.get('fib_61.8', current_price * 1.382)  # Adjusted for SELL
                if current_price > ote_level:
                    stop_loss = min(stop_loss, ote_level)
            
            return stop_loss, take_profit
            
        except Exception as e:
            self.logger_trading.error(f"Fibonacci stop loss/take profit calculation failed: {e}")
            return None, None

    def _generate_emergency_timeframe_data(self, timeframe):
        """Generate emergency fallback data for main timeframe"""
        try:
            self.logger_system.warning(f"Generating emergency data for timeframe {timeframe}")
            
            # Get current price as base
            current_price = 67000.0  # Default price
            try:
                if not self.config.simulation_mode:
                    ticker = self.exchange.fetch_ticker(self.config.symbol)
                    current_price = ticker['last'] if ticker and ticker['last'] else current_price
                else:
                    # Generate random price in simulation mode
                    import random
                    current_price = 67000.0 + random.uniform(-500, 500)
            except Exception as e:
                self.logger_system.warning(f"Fetch current price failed, using default: {e}")
            
            # Generate minimal usable OHLCV data (3 data points)
            ...
            # Generate price data (small fluctuation)
            import random
            ohlcv_data = []
            for i, ts in enumerate(timestamps):
                # Generate small price fluctuation
                price_variation = current_price * random.uniform(-0.01, 0.01)  # ±1%
                base_price = current_price + price_variation
                
                high = base_price * random.uniform(1.001, 1.005)  # 0.1-0.5% up
                low = base_price * random.uniform(0.995, 0.999)  # 0.1-0.5% down
    ...
    # New: FVG Detection - Enhanced for stacking
    ...
    def calculate_fvg_levels(self, fvgs: List[Dict[str, Any]]) -> Dict[str, float]:
        """Calculate key FVG levels, check for stacking - Optimization 1: Enforce stack threshold"""
        if not fvgs:
            return {}
        ...
        # Optimization 1: Enforce threshold - if stack < threshold, return empty (no valid FVG)
        if stack_bull < self.config.fvg_stack_threshold and stack_bear < self.config.fvg_stack_threshold:
            self.logger_trading.debug(f"FVG stack below threshold ({stack_bull}/{stack_bear} < {self.config.fvg_stack_threshold}), invalidating levels")
            return {}
    ...
    def calculate_key_levels(self, multi_tf_data: Dict[str, pd.DataFrame]) -> Dict[str, float]:
        try:
            # Safely get timeframe data, avoid KeyError
            ...
            if daily_df.empty:
                self.logger_system.warning("Daily data missing, using fallback mechanism")
            if h4_df.empty:
                self.logger_system.warning("4h data missing, using fallback mechanism")
            if h1_df.empty:
                self.logger_system.warning("1h data missing, skip 1h EMA calculation")
            if m15_df.empty:
                self.logger_system.warning("15m data missing, using fallback mechanism")
            ...
            # Calculate levels for each timeframe, return empty dict if data missing
            ...
            # New: FVG levels from daily, 4h, 1h, 15m - HTF/LTF alignment check
            ...
            # Optimization 1: Only include if HTF FVG aligns with LTF (e.g., both have valid stacks)
            if h4_fvg_levels and m15_fvg_levels:
                # Simple alignment: check if midpoints within 0.5%
                h4_mid = (h4_fvg_levels.get('fvg_bull_mid', 0) + h4_fvg_levels.get('fvg_bear_mid', 0)) / 2
                ltf_mid = (m15_fvg_levels.get('fvg_bull_mid', 0) + m15_fvg_levels.get('fvg_bear_mid', 0)) / 2
                if abs(h4_mid - ltf_mid) / h4_mid > 0.005:  # >0.5% no alignment
                    h4_fvg_levels = {}
                    self.logger_trading.debug("FVG HTF/LTF misalignment, invalidating 4h levels")
    ...
    def calculate_hourly_ema_levels(self, hourly_df: pd.DataFrame) -> Dict[str, float]:
        """Calculate 1h EMA levels - EMA21/55/100/200, dynamic based on data volume"""
        try:
            # Check if data empty
            if hourly_df.empty:
                self.logger_system.warning("1h data empty, unable to calculate EMA levels")
                return {}
            
            data_length = len(hourly_df)
            self.logger_system.debug(f"1h data length: {data_length}")
            
            # Dynamically calculate available EMA indicators based on data length
            ema_levels = {}
            
            # EMA21 - Min 21 candles needed
            if data_length >= 21:
                try:
                    ema_21 = hourly_df['close'].ewm(span=21).mean().iloc[-1]
                    ema_levels['ema_21_1h'] = round(float(ema_21), 2)
                    self.logger_system.debug(f"EMA21(1h) calculation successful: {ema_levels['ema_21_1h']}")
                except Exception as e:
                    self.logger_system.warning(f"EMA21(1h) calculation failed: {e}")
            else:
                self.logger_system.warning(f"Insufficient data for EMA21(1h), need 21 candles, current {data_length}")
            
            # EMA55 - Min 55 candles needed
            if data_length >= 55:
                try:
                    ema_55 = hourly_df['close'].ewm(span=55).mean().iloc[-1]
                    ema_levels['ema_55_1h'] = round(float(ema_55), 2)
                    self.logger_system.debug(f"EMA55(1h) calculation successful: {ema_levels['ema_55_1h']}")
                except Exception as e:
                    self.logger_system.warning(f"EMA55(1h) calculation failed: {e}")
            else:
                self.logger_system.warning(f"Insufficient data for EMA55(1h), need 55 candles, current {data_length}")
            
            # EMA100 - Min 100 candles needed
            if data_length >= 100:
                try:
                    ema_100 = hourly_df['close'].ewm(span=100).mean().iloc[-1]
                    ema_levels['ema_100_1h'] = round(float(ema_100), 2)
                    self.logger_system.debug(f"EMA100(1h) calculation successful: {ema_levels['ema_100_1h']}")
                except Exception as e:
                    self.logger_system.warning(f"EMA100(1h) calculation failed: {e}")
            else:
                self.logger_system.warning(f"Insufficient data for EMA100(1h), need 100 candles, current {data_length}")
            
            # EMA200 - Min 200 candles needed
            if data_length >= 200:
                try:
                    ema_200 = hourly_df['close'].ewm(span=200).mean().iloc[-1]
                    ema_levels['ema_200_1h'] = round(float(ema_200), 2)
                    self.logger_system.debug(f"EMA200(1h) calculation successful: {ema_levels['ema_200_1h']}")
                except Exception as e:
                    self.logger_system.warning(f"EMA200(1h) calculation failed: {e}")
            else:
                self.logger_system.warning(f"Insufficient data for EMA200(1h), need 200 candles, current {data_length}")
            
            if ema_levels:
                self.logger_system.info(f"1h EMA levels calculation completed, total {len(ema_levels)} indicators")
            else:
                self.logger_system.warning("All 1h EMA indicators calculation failed, data length insufficient")
            
            return ema_levels
            
        except Exception as e:
            self.logger_system.exception(f"1h EMA levels calculation exception: {e}")
            return {}

    def calculate_daily_liquidity_levels(self, daily_df: pd.DataFrame) -> Dict[str, float]:
        """Calculate daily liquidity levels - Monday open, previous week high/low"""
        if len(daily_df) < 10:
            return {}
        
        # Get recent Monday open
        daily_df_copy = daily_df.copy()
        daily_df_copy['weekday'] = pd.to_datetime(daily_df_copy.index).weekday
        
        # Find recent Monday (weekday=0)
    ...
    # New: Enhanced candle pattern detection - Optimization 4: Already supports Doji/Hammer
    ...
    def get_fallback_levels(self) -> Dict[str, float]:
        try:
            ticker = self.safe_fetch_ticker(exchange, self.config.symbol)
            current_price = round(ticker['last'], 2) if ticker else 67000.0
            return {
    ...
    def check_price_activation(self, current_price: float, key_levels: Dict[str, float]) -> Tuple[bool, Optional[str]]:
        ...
        for level_name in sorted_priority:
            ...
            if interactions > self.config.max_zone_interactions:
                self.logger_trading.debug(f"Key level {level_name} interacted {interactions} times (> {self.config.max_zone_interactions}), skip")
                continue
            
            # Check 5-minute cooldown for this key level
            with self.lock:
                last_level_activation = self.level_activation_times.get(level_name, 0)
                if current_time - last_level_activation < 300:  # 5 minutes = 300 seconds
                    self.logger_trading.debug(f"Key level {level_name} still in cooldown, skip activation check")
                    continue
            ...
            if distance < adjusted_threshold:
                ...
                # Log activation, include threshold accumulation and win rate info
                ...
                self.logger_trading.info(f"Key level activation: {level_name}")
                self.logger_trading.info(f"  Current price: {current_price:.4f}")
                self.logger_trading.info(f"  Key level price: {level:.4f}")
                self.logger_trading.info(f"  Distance: {distance*100:.3f}%")
                self.logger_trading.info(f"  Activation threshold: {adjusted_threshold*100:.3f}%")
                self.logger_trading.info(f"  Cumulative activations: {stats['activations']}")
                self.logger_trading.info(f"  Average threshold accumulation: {avg_threshold*100:.3f}%")
                self.logger_trading.info(f"  Win rate: {win_rate:.1f}% ({stats['successful_trades']}/{stats['successful_trades'] + stats['failed_trades']})")
    ...
    def get_adjusted_threshold(self, level_name: str) -> float:
        """Return activation threshold based on key level type - Per README.md rules"""
        # Directly use absolute thresholds specified in README.md - New weight system, adjusted for aggressive intraday
        absolute_thresholds = {
            # Daily levels (highest weight - institutional positions): Reduced 15-20% to capture more weekly/daily liquidity hunt opportunities
            'monday_open': 0.0010,          # 0.10% (from 0.12%, increase institutional open reversal signals)
            'daily_open': 0.00085,          # 0.085% (from 0.10%, optimize intraday open capture)
            'prev_week_high': 0.00075,      # 0.075% (from 0.09%, more weekly high breakout/reversal)
            'prev_week_low': 0.00075,       # 0.075% (from 0.09%, same)
            'daily_vwap': 0.00065,          # 0.065% (from 0.08%, VWAP as intraday hub more sensitive)
            'daily_fvg_bull_mid': 0.00075,  # 0.075% (new, daily FVG hub, high weight sensitivity)
            'daily_fvg_bear_mid': 0.00075,  # 0.075% (new, same)
            'recent_10d_high': 0.00075,     # 0.075% (from 0.09%, similar to weekly, increase 10d trend opportunities)
            'recent_10d_low': 0.00075,      # 0.075% (from 0.09%, same)
            'daily_ob_bull': 0.00065,       # 0.065% (from 0.08%, bull OB easier to activate long)
            'daily_ob_bear': 0.00065,       # 0.065% (from 0.08%, bear OB easier to activate short)
            
            # 4h levels (high weight - mid-term structure positions): Reduced 15% to bridge daily-hour, increase mid-term confluence
            '4h_high': 0.00065,             # 0.065% (from 0.08%, more 4h high liquidity grab)
            '4h_low': 0.00065,              # 0.065% (from 0.08%, same)
            '4h_ob_bull': 0.00055,          # 0.055% (from 0.07%, OB confirmation after entry more frequent)
            '4h_ob_bear': 0.00055,          # 0.055% (from 0.07%, same)
            '4h_fvg_bull_mid': 0.00055,     # 0.055%
            '4h_fvg_bear_mid': 0.00055,     # 0.055%
            '4h_gap_up': 0.0005,            # 0.05% (from 0.06%, more gap fill opportunities)
            '4h_gap_down': 0.0005,          # 0.05% (from 0.06%, same)
            'ema_21_4h': 0.00055,           # 0.055% (from 0.07%, short EMA more sensitive trend turn)
            'ema_55_4h': 0.00055,           # 0.055% (from 0.07%, same)
            'ema_200_4h': 0.00065,          # 0.065% (from 0.08%, long EMA as support easier to touch)
            'ema_100_4h': 0.00055,          # 0.055% (from 0.07%, similar to OB)
            
            # 1h levels (medium weight - short-term structure positions): Reduced 10% for precise positioning, combined with 15m execution
            'ema_200_1h': 0.0005,           # 0.05% (from 0.06%, long EMA trend confirmation more)
            'ema_100_1h': 0.00045,          # 0.045% (from 0.055%, mid EMA optimize short pullback)
            'ema_55_1h': 0.0004,            # 0.04% (from 0.05%, increase mid EMA opportunities)
            'ema_21_1h': 0.00015,           # 0.015% (from 0.02%, further aggressive short EMA breakout)
            '1h_ob_bull': 0.0004,           # 0.04%
            '1h_ob_bear': 0.0004,           # 0.04%
            '1h_fvg_bull_mid': 0.00035,     # 0.035%
            '1h_fvg_bear_mid': 0.00035,     # 0.035%
            
            # 15m levels (execution level - precise entry positions): Fine-tune 10%, already low, avoid excessive noise; use momentum filter
            '15m_liquidity_hunt': 0.00035,  # 0.035% (from 0.04%, slightly increase hunt signals)
            '15m_structure_reversal': 0.00025,  # 0.025% (from 0.03%, reversal confirmation more)
            '15m_structure_break': 0.00008,    # 0.008% (from 0.01%, extremely aggressive breakout, but need R:R >2.5 filter)
        ...
        # Return specified threshold, if not found use default
    ...
    def calculate_15min_amplitude(self, m15_df: pd.DataFrame) -> float:
        """Calculate latest 15m candle amplitude percentage"""
        try:
            if len(m15_df) == 0:
                return 0.0
            ...
            # Calculate amplitude percentage (high - low) / low
            amplitude_pct = (high - low) / low if low > 0 else 0.0
            
            self.logger_system.info(f"Latest 15m candle amplitude: {amplitude_pct*100:.3f}% (High: {high}, Low: {low})")
            return amplitude_pct
            
        except Exception as e:
            self.logger_system.error(f"Calculate 15m amplitude failed: {e}")
            return 0.0

    def _get_last_timestamp(self, df: pd.DataFrame) -> Optional[datetime]:
        """Get DataFrame's last timestamp"""
        try:
            if df.empty:
                return None
    ...
    def get_kline_structure_summary(self, df: pd.DataFrame, tf_name: str, lookback: int = 20) -> Dict[str, Any]:
        """Get K-line structure summary - Enhanced with candle patterns and FVG"""
        try:
            if len(df) < 5:
    ...
    def _generate_simulation_data(self) -> Dict[str, Any]:
        ...
        self.logger_system.info("Simulation mode: Generate simulation data")
        ...
        # Generate sufficient data for each timeframe
        for tf in self.config.timeframes:
            # Generate sufficient data points
            data_points = 100  # Sufficient data points
            
            # Generate time series - Use dynamic mapping
            now = datetime.now(timezone.utc)
            timestamps = []
            
            # Timeframe to time increment mapping - FIXED: Medium 7
            timeframe_deltas = {
            ...
            # Get corresponding time increment, default to 1 minute if not exist
        ...
        # Generate simulated structure summary
        structures_summary = {}
        for tf in self.config.timeframes:
        ...
        return {
            ...
            'current_price': current_price,
            'price': current_price,  # Add price field to match code expectation
            'key_levels': key_levels,
            'amplitude': amplitude_data,  # Fix field name
            'amplitude_data': amplitude_data,
            'timeframes_fetched': list(multi_tf_data.keys()),
            'structures_summary': structures_summary,  # Add structure summary
    ...
    def get_multi_timeframe_data(self) -> Optional[Dict[str, Any]]:
        try:
            # Generate simulation data in simulation mode
            if self.config.simulation_mode:
                return self._generate_simulation_data()
            
            multi_tf_data = {}
            fetch_timeframes = self.config.timeframes + ['5m']  # Add 5m to support momentum filter
            
            # Log different info based on data source
            if self.config.data_source == 'websocket':
                self.logger_system.info("Starting to fetch multi-timeframe data from WebSocket indicators")
            else:
                self.logger_system.info("Starting to fetch multi-timeframe data directly from Hyperliquid API")
            
            for tf in fetch_timeframes:
                try:
                    # Dynamic limit: 15m use 1000, others 200
                    limit = 1000 if tf == '15m' else 200
                    self.logger_system.info(f"Fetching {tf} data: limit={limit}")
                    
                    # Unified use safe_fetch_ohlcv method, automatically selects data source by priority
                    ohlcv = self.safe_fetch_ohlcv(exchange, self.config.symbol, tf, limit)
                    
                    if not ohlcv or len(ohlcv) < 50:  # At least 50 points
                        self.logger_system.warning(f"{tf} data insufficient ({len(ohlcv) if ohlcv else 0} < 50), skip")
                        continue
                    ...
                    if df['close'].isna().all():
                        self.logger_system.warning(f"{tf} close column all NaN, skip")
                        continue
                    
                    # Calculate technical indicators
                    df = self.calculate_technical_indicators(df)
                    multi_tf_data[tf] = df
                    self.logger_system.info(f"{tf} data processing completed, total {len(df)} data points")
                    
                except Exception as e:
                    self.logger_system.exception(f"Fetch {tf} data failed: {e}")
                    continue
            
            # Ensure at least primary TF available
            if self.config.primary_timeframe not in multi_tf_data:
                self.logger_system.error(f"Primary timeframe {self.config.primary_timeframe} fetch failed")
                return None
            ...
            # Calculate amplitude (use 1d data)
            daily_df = multi_tf_data.get('1d')
            if daily_df is not None and len(daily_df) > 0:
                ...
                amplitude = {'avg_amplitude': 3500.0, 'expected_rr_range': 2800.0}  # BTC typical value
    ...
    def get_current_position(self) -> Optional[PositionInfo]:
        if self.config.simulation_mode:
            return None  # Return empty position in simulation mode
    ...
    def _calculate_confidence_score(self, multi_tf_data: Dict[str, pd.DataFrame], 
        ...
        Based on technical indicators, Fibonacci analysis, risk/reward ratio, etc.
        """
        try:
            score = 0.0
            
            # 1. Risk/reward ratio score (0-25 points)
            if rr_ratio >= 3.0:
                score += 25
            elif rr_ratio >= 2.5:
                score += 20
            ...
            # 2. Fibonacci optimization score (0-20 points)
            if fib_stop_loss and fib_take_profit:
                score += 20
                self.logger_system.info("Fibonacci optimization bonus: +20")
            
            # 3. Multi-timeframe technical indicator consistency score (0-30 points)
            tf_scores = []
            for tf, df in multi_tf_data.items():
                if df is not None and len(df) > 0:
                    tf_score = self._evaluate_timeframe_indicators(df, current_price)
                    tf_scores.append(tf_score)
                    self.logger_system.info(f"{tf} timeframe technical indicator score: {tf_score}")
            
            if tf_scores:
                avg_tf_score = sum(tf_scores) / len(tf_scores)
                score += avg_tf_score * 0.3  # Max 30 points
            
            # 4. Trend consistency score (0-15 points)
            trend_consistency = self._evaluate_trend_consistency(multi_tf_data, current_price)
            score += trend_consistency
            
            # 5. Volatility and liquidity score (0-10 points)
            volatility_score = self._evaluate_volatility_conditions(multi_tf_data)
            score += volatility_score
            
            # New: Confirmation signals boost - Optimization 4
            # Volume confirmation
            m15_df = multi_tf_data.get('15m', pd.DataFrame())
            ...
            if patterns:
                score += 8 * len(patterns)  # Up to +24 for multiple patterns - dynamic weight
                self.logger_system.info(f"Candle patterns boost: +{8 * len(patterns)}")
            
            # FVG stacking - Use passed key_levels (fixed)
            ...
            # Ensure score in 0-100 range
            score = max(0, min(100, score))
            
            self.logger_system.info(f"Total confidence score: {score:.2f}/100 (R:R={rr_ratio:.2f}, Fibonacci={'Yes' if fib_stop_loss else 'No'})")
            return score
            
        except Exception as e:
            self.logger_system.error(f"Calculate confidence score error: {e}")
            return 30.0  # Default medium-low score

    def _evaluate_timeframe_indicators(self, df: pd.DataFrame, current_price: float) -> float:
        """Evaluate single timeframe technical indicator strength (0-100)"""
        try:
            if len(df) < 10:
                return 50.0
            ...
            if 'rsi' in df.columns and not df['rsi'].isna().all():
                rsi = df['rsi'].iloc[-1]
                if 30 <= rsi <= 70:  # Neutral range
                    score += 70
                elif 20 <= rsi <= 80:  # Good range
                    score += 50
            ...
            if 'macd' in df.columns and 'macd_signal' in df.columns:
                if not df['macd'].isna().all() and not df['macd_signal'].isna().all():
                    ...
                    if macd > macd_signal:  # Bull signal
                        score += 60
                    else:  # Bear signal
                        score += 40
                    indicators_count += 1
            ...
            if all(col in df.columns for col in ['bb_upper', 'bb_lower', 'bb_middle']):
                ...
                if not pd.isna(bb_upper) and not pd.isna(bb_lower):
                    if bb_lower <= current_price <= bb_upper:  # Within Bollinger Bands
                        if abs(current_price - bb_middle) / (bb_upper - bb_lower) < 0.3:
                            score += 70  # Close to middle band
                        else:
                            score += 50  # Within band but deviated from middle
                    else:
                        score += 30  # Breakout of Bollinger Bands
                    indicators_count += 1
            ...
            if 'ema_20' in df.columns and 'ema_50' in df.columns:
                if not df['ema_20'].isna().all() and not df['ema_50'].isna().all():
                    ...
                    if ema_20 > ema_50:  # Short EMA above long EMA
                        score += 60
                    else:
                        score += 40
                    indicators_count += 1
            ...
            self.logger_system.error(f"Evaluate timeframe indicators error: {e}")
            return 50.0

    def _evaluate_trend_consistency(self, multi_tf_data: Dict[str, pd.DataFrame], current_price: float) -> float:
        """Evaluate multi-timeframe trend consistency (0-15 points) - Emphasize higher TF bias"""
        try:
            trend_signals = []
            ...
            if consistency_ratio >= 0.8:  # 80%+ consistent
                return 15.0
            elif consistency_ratio >= 0.6:  # 60%+ consistent
                return 12.0
            elif consistency_ratio >= 0.4:  # 40%+ consistent
                return 8.0
            else:
                return 5.0
            
        except Exception as e:
            self.logger_system.error(f"Evaluate trend consistency error: {e}")
            return 7.5

    def _evaluate_volatility_conditions(self, multi_tf_data: Dict[str, pd.DataFrame]) -> float:
        """Evaluate volatility conditions (0-10 points)"""
        try:
            # Use 15m data to evaluate volatility
            m15_df = multi_tf_data.get('15m')
            if m15_df is None or len(m15_df) < 20:
                return 5.0
            
            # Calculate ATR relative volatility
            if 'atr' in m15_df.columns and not m15_df['atr'].isna().all():
                ...
                # Moderate volatility scores highest
            ...
            self.logger_system.error(f"Evaluate volatility conditions error: {e}")
            return 5.0

    def _determine_confidence_level(self, confidence_score: float) -> Tuple[str, bool]:
        """
        Determine confidence level and whether to encourage trading based on confidence score
        Return: (confidence level, encourage trading)
        """
    ...
    def _determine_signal_direction(self, df: pd.DataFrame, current_price: float) -> str:
        """
        Judge signal direction based on technical indicators
        For Fibonacci analysis trend judgment
        """
        try:
            if len(df) < 10:
                return 'HOLD'
            ...
            # Need at least 2 signals confirmation
            ...
            self.logger_system.warning(f"Signal direction judgment failed: {e}")
            return 'HOLD'

    def create_fallback_signal(self, price_data):
        # ENHANCED: Integrate Fibonacci and technical analysis for optimized stop profit/loss
        try:
            # Multi-layer safety check to ensure price_data valid
            ...
            # Try to get ATR from technical data
            atr = None
            ...
            # Try using Fibonacci analysis to optimize stop profit/loss
            fib_stop_loss = None
            fib_take_profit = None
            
            # Check if multi-timeframe data available for Fibonacci analysis
            if 'multi_tf_data' in price_data and price_data['multi_tf_data']:
                # Prioritize 15m data for Fibonacci analysis
                for tf in ['15m', '1h']:
                    if tf in price_data['multi_tf_data']:
                        df = price_data['multi_tf_data'][tf]
                        if len(df) >= 20:  # Ensure sufficient data
                            try:
                                # Judge trend direction based on technical indicators
                                ...
                                if signal_direction != 'HOLD':
                                    ...
                                    if fib_sl and fib_tp:
                                        ...
                                        self.logger_system.info(f"Using Fibonacci analysis({tf}) to optimize stop profit/loss: SL={fib_sl:.2f}, TP={fib_tp:.2f}")
                                        break
                            except Exception as e:
                                self.logger_system.warning(f"Fibonacci analysis failed({tf}): {e}")
                                continue
            
            # Calculate basic ATR stop profit/loss
            atr_stop_loss = round(current - atr * 2, 2)
            atr_take_profit = round(current + atr * 3, 2)
            
            # Select optimal stop profit/loss strategy
            if fib_stop_loss and fib_take_profit:
                # Validate reasonableness of Fibonacci stop profit/loss
                sl_distance = abs(current - fib_stop_loss)
                tp_distance = abs(fib_take_profit - current)
                
                # Ensure stop profit/loss distance in reasonable range
                if (0.5 * atr <= sl_distance <= 4 * atr and 
                    1 * atr <= tp_distance <= 6 * atr and
                    tp_distance / sl_distance >= 1.5):  # Min risk/reward ratio 1.5:1
                    
                    stop_loss = round(fib_stop_loss, 2)
                    take_profit = round(fib_take_profit, 2)
                    self.logger_system.info(f"Adopt Fibonacci optimized stop profit/loss: SL={stop_loss}, TP={take_profit}")
                else:
                    # Fibonacci result unreasonable, use ATR method
                    stop_loss = atr_stop_loss
                    take_profit = atr_take_profit
                    self.logger_system.info(f"Fibonacci result unreasonable, use ATR stop profit/loss: SL={stop_loss}, TP={take_profit}")
            else:
                # No Fibonacci data, use ATR method
                stop_loss = atr_stop_loss
                take_profit = atr_take_profit
                self.logger_system.info(f"No Fibonacci data, use ATR stop profit/loss: SL={stop_loss}, TP={take_profit}")
            ...
            # Ensure stop loss/take profit logic correct
            ...
            # Calculate trade confidence score - Pass key_levels
            # Extract multi-timeframe data from price_data, if not use empty dict
            multi_tf_data = price_data.get('multi_tf_data', {})
            key_levels = price_data.get('key_levels', {})  # New: Get from price_data
            confidence_score = self._calculate_confidence_score(
                multi_tf_data, current, fib_stop_loss, fib_take_profit, rr_ratio, key_levels  # Add key_levels
            )
            ...
            fallback_signal = {
                ...
                "stop_loss": float(stop_loss),  # Ensure float type
                "take_profit": float(take_profit),  # Ensure float type
            ...
            # Final validation of fallback_signal completeness
            ...
            # Emergency backup signal, ensure system won't crash
            emergency_signal = {
                "signal": "HOLD",
    ...
    def _generate_compact_smc_summary(self, price_data: Dict[str, Any]) -> str:
        """Generate simplified SMC analysis summary for DeepSeek AI analysis - Enhanced with FVG and patterns"""
        try:
            ...
            # Simplified timeframe structure analysis
            tf_summary = []
            ...
            # Combine simplified summary
            summary_parts = []
            if tf_summary:
                summary_parts.append(f"Structure: {' | '.join(tf_summary[:2])}")  # Show only first 2 timeframes
            if key_levels_text:
                summary_parts.append(f"Levels: {' | '.join(key_levels_text[:3])}")  # Show only first 3 key levels
            if ema_text:
                summary_parts.append(f"EMA1H: {' | '.join(ema_text[:2])}")  # Show first 2 1h EMAs
            if ob_text:
                summary_parts.append(f"OB: {' | '.join(ob_text[:2])}")  # Show only first 2 order blocks
            if fvg_text:
            ...
            self.logger_system.warning(f"Generate SMC summary error: {e}")
    ...
    def safe_json_parse(self, json_str: str) -> Optional[Dict[str, Any]]:
        """Safely parse JSON string, support multiple format fixes and candidate selection"""
        try:
            result = json.loads(json_str)
            # Ensure result contains signal field
            ...
            try:
                ...
                # If candidates found, select highest confidence
                if candidates:
                    ...
                    if sl is None or tp is None:
                        self.logger_system.warning(f"DeepSeek returned null values: SL={sl}, TP={tp}")
                        self.logger_system.warning(f"Full candidate: {best}")
                    elif not isinstance(sl, (int, float)) or not isinstance(tp, (int, float)):
                        self.logger_system.warning(f"DeepSeek returned non-numeric type: SL={type(sl).__name__}({sl}), TP={type(tp).__name__}({tp})")
                        self.logger_system.warning(f"Full candidate: {best}")
    ...
    def analyze_with_deepseek(self, price_data: Dict[str, Any], activated_level: Optional[str] = None) -> Dict[str, Any]:
        ...
        if self._should_check_api_health('deepseek'):
            self.logger_system.info("Performing DeepSeek API health check...")
            self._check_deepseek_health()
        
        # If API status unhealthy and consecutive failures too many, directly use backup signal
        if (self.api_health_status['deepseek']['status'] == 'unhealthy' and 
            self.api_health_status['deepseek']['consecutive_failures'] >= 3):
            self.logger_system.warning(f"DeepSeek API consecutive failures {self.api_health_status['deepseek']['consecutive_failures']} times, directly use backup signal")
            return self.create_fallback_signal(price_data)
        
        # Generate simplified SMC analysis text
        smc_summary = self._generate_compact_smc_summary(price_data)
        
        # Simplified K-line data
        m15_df = price_data['multi_tf_data']['15m']
        recent_klines = m15_df.tail(3)  # Reduced to 3 candles
        ...
        threshold_info = ""
        if activated_level:
            # Get activation threshold info as reference
        ...
        conf_rules = f"Confirmations: Volume >{self.config.volume_confirmation_threshold}x MA, Candle patterns (e.g., Engulfing), FVG stacking >= {self.config.fvg_stack_threshold}. Fresh zones only (interactions <= {self.config.max_zone_interactions})."
        ...
        # Improved prompt, ensure return valid numeric - Enhanced for new features
        current_price = price_data['price']
        prompt = f"""SMC/ICT Analysis for {self.config.symbol.split(':')[0]}:
Price: ${current_price:,.0f} | {kline_summary}
Position: {position_text} | {signal_text} | {activation_text}
{smc_summary}
{rules}
Aggressive Day Trading: Focus 15m/5m momentum, enter on 1.2:1 R:R, ignore higher TF if price action strong.
EMPHASIS: Higher TF ({higher_tf}) bias for direction + Lower TF ({lower_tf}) entry. Require confirmations: Volume >{self.config.volume_confirmation_threshold}x MA, candle patterns, FVG stacking. Use fresh zones only (no multiple interactions).
CRITICAL: Always provide EXACT numeric values for stop_loss and take_profit. Never use null, variables, or placeholders.
For BUY signals: stop_loss < {current_price:.0f} < take_profit
For SELL signals: take_profit < {current_price:.0f} < stop_loss
For HOLD signals: Use current price ±1% as SL/TP
Example valid responses:
BUY: {{"signal": "BUY", "reason": "Bullish structure", "stop_loss": {current_price*0.98:.0f}, "take_profit": {current_price*1.04:.0f}, "confidence": "MEDIUM"}}
SELL: {{"signal": "SELL", "reason": "Bearish structure", "stop_loss": {current_price*1.02:.0f}, "take_profit": {current_price*0.96:.0f}, "confidence": "MEDIUM"}}
HOLD: {{"signal": "HOLD", "reason": "Neutral structure", "stop_loss": {current_price*0.99:.0f}, "take_profit": {current_price*1.01:.0f}, "confidence": "LOW"}}
Return JSON only:
{{
 "signal": "BUY|SELL|HOLD",
 "reason": "Brief SMC analysis",
 ...
 "confidence": "HIGH|MEDIUM|LOW"
        ...
        for attempt in range(max_retries):
            try:
                ...
                # Log full request info sent to DeepSeek
                self.logger_api.info(f"=== DeepSeek API Request (attempt {attempt+1}/{max_retries}) ===")
                self.logger_api.info(f"Model: deepseek-chat")
                self.logger_api.info(f"Temperature: {self.config.temperature}")
                self.logger_api.info(f"Timeout: {timeout}s")
                self.logger_api.info(f"System prompt: SMC/ICT expert. Return strict JSON only.")
                self.logger_api.info(f"User prompt content:\n{prompt}")
                self.logger_api.info("=" * 50)
                
                response = deepseek_client.chat.completions.create(
                    model="deepseek-chat",
                    messages=[
                        {"role": "system", "content": "SMC/ICT expert. Return strict JSON only."},
                ...
                # Log full API response info
                result = response.choices[0].message.content
                self.logger_api.info(f"=== DeepSeek API Response (attempt {attempt+1}) ===")
                self.logger_api.info(f"Response status: Success")
                self.logger_api.info(f"Response length: {len(result)} characters")
                self.logger_api.info(f"Raw response content:\n{result}")
                self.logger_api.info("=" * 50)
                
                signal_data = self.safe_json_parse(result)
                
                # Log JSON parse result
                self.logger_api.info(f"=== JSON Parse Result (attempt {attempt+1}) ===")
                if signal_data is not None:
                    self.logger_api.info(f"Parse status: Success")
                    self.logger_api.info(f"Parsed data: {signal_data}")
                    
                    # Validate required fields
                    required_fields = ['signal', 'reason', 'stop_loss', 'take_profit', 'confidence']
                    missing_fields = [field for field in required_fields if field not in signal_data]
                    
                    self.logger_api.info(f"Field validation: Required fields {required_fields}")
                    if missing_fields:
                        self.logger_api.warning(f"Missing fields: {missing_fields}")
                    else:
                        self.logger_api.info(f"Field validation: Passed")
                    ...
                    if not missing_fields:
                        ...
                        self.logger_api.info(f"=== Stop Loss/Take Profit Validation (attempt {attempt+1}) ===")
                        self.logger_api.info(f"Signal type: {signal_type}")
                        self.logger_api.info(f"Current price: {current_price}")
                        self.logger_api.info(f"Stop loss price: {sl} (type: {type(sl).__name__})")
                        self.logger_api.info(f"Take profit price: {tp} (type: {type(tp).__name__})")
                        
                        # Detailed validation logic
                        validation_errors = []
                        
                        if sl is None:
                            validation_errors.append("Stop loss price is None")
                        elif not isinstance(sl, (int, float)):
                            validation_errors.append(f"Stop loss price type error: {type(sl).__name__}({sl})")
                        elif sl <= 0:
                            validation_errors.append(f"Stop loss price invalid: {sl} <= 0")
                        
                        if tp is None:
                            validation_errors.append("Take profit price is None")
                        elif not isinstance(tp, (int, float)):
                            validation_errors.append(f"Take profit price type error: {type(tp).__name__}({tp})")
                        elif tp <= 0:
                            validation_errors.append(f"Take profit price invalid: {tp} <= 0")
                        
                        # Check price logic
                        if isinstance(sl, (int, float)) and isinstance(tp, (int, float)):
                            if signal_type == 'BUY':
                                if sl >= current_price:
                                    validation_errors.append(f"BUY signal stop loss price error: {sl} >= {current_price}")
                                if tp <= current_price:
                                    validation_errors.append(f"BUY signal take profit price error: {tp} <= {current_price}")
                            elif signal_type == 'SELL':
                                if sl <= current_price:
                                    validation_errors.append(f"SELL signal stop loss price error: {sl} <= {current_price}")
                                if tp >= current_price:
                                    validation_errors.append(f"SELL signal take profit price error: {tp} >= {current_price}")
                            elif signal_type == 'HOLD':
                                # HOLD signal SL/TP validation relatively loose
                                pass
                        
                        if validation_errors:
                            self.logger_api.error(f"=== Validation Failed (attempt {attempt+1}) ===")
                            self.logger_api.error(f"Validation errors: {'; '.join(validation_errors)}")
                            self.logger_api.error(f"Signal type: {signal_type}")
                            self.logger_api.error(f"Current price: {current_price}, SL: {sl}, TP: {tp}")
                            self.logger_api.error(f"Full signal data: {signal_data}")
                            self.logger_api.error("=" * 50)
                            signal_data = None
                        else:
                            self.logger_api.info(f"=== Validation Success (attempt {attempt+1}) ===")
                            self.logger_api.info(f"Signal valid: {signal_data['signal']}")
                            self.logger_api.info(f"Stop loss: {sl}, Take profit: {tp}")
                            self.logger_api.info(f"Confidence: {signal_data.get('confidence', 'N/A')}")
                            self.logger_api.info(f"Reason: {signal_data.get('reason', 'N/A')}")
                            self.logger_api.info("=" * 50)
                            # Update API success status
                            self._update_api_success('deepseek')
                            break
                    else:
                        self.logger_api.warning(f"Field validation failed: Missing fields {missing_fields}")
                        signal_data = None
                else:
                    # JSON parse failed
                    self.logger_api.error(f"Parse status: Failed")
                    self.logger_api.error(f"Raw response content: {result[:500]}...")  # Show only first 500 characters
                    self.logger_api.error("=" * 50)
        ...
        # Handle logic after loop ends
        if signal_data is None:
            # Detailed error diagnostic log
            api_status = self.api_health_status['deepseek']
            self.logger_system.warning(f"DeepSeek API complete failure, use backup signal")
            self.logger_system.warning(f"API health status: {api_status['status']}, Consecutive failures: {api_status['consecutive_failures']} times")
            self.logger_system.warning(f"Last success time: {api_status.get('last_success_time', 'Unknown')}")
            self.logger_system.warning(f"Last error: {api_status.get('last_error', 'Unknown')}")
            
            # Provide diagnostic suggestions
            if api_status['consecutive_failures'] >= 5:
                self.logger_system.error("DeepSeek API consecutive failures too many, suggest checking:")
                self.logger_system.error("1. Network connection normal?")
                self.logger_system.error("2. API key valid?")
                self.logger_system.error("3. DeepSeek service normal?")
                self.logger_system.error("4. Request frequency exceed limit?")
        ...
        if not all(field in signal_data for field in required_fields):
            missing_fields = [field for field in required_fields if field not in signal_data]
            self.logger_system.warning(f"Signal data missing required fields: {missing_fields}")
            self.logger_system.warning(f"Raw signal data: {signal_data}")
            self.logger_system.warning("Use backup signal instead")
            signal_data = self.create_fallback_signal(price_data)
        
        # Extra validation stop loss/take profit not None
        if signal_data.get('stop_loss') is None or signal_data.get('take_profit') is None:
            self.logger_system.warning(f"Stop loss/take profit value None - SL: {signal_data.get('stop_loss')}, TP: {signal_data.get('take_profit')}")
            self.logger_system.warning(f"Current price: {price_data['price']}")
            self.logger_system.warning("This may be DeepSeek API returned invalid data, use backup signal")
            signal_data = self.create_fallback_signal(price_data)
        
        # Validate stop loss/take profit as valid numeric
        try:
            ...
            if sl <= 0 or tp <= 0:
                self.logger_system.warning(f"Stop loss/take profit value invalid - SL: {sl}, TP: {tp} (must >0)")
                self.logger_system.warning(f"Current price: {current_price}")
                self.logger_system.warning("Use backup signal recalculate")
                signal_data = self.create_fallback_signal(price_data)
            elif abs(sl - current_price) / current_price > 0.1 or abs(tp - current_price) / current_price > 0.1:
                self.logger_system.warning(f"Stop loss/take profit value abnormal - SL: {sl}, TP: {tp}, Current price: {current_price}")
                self.logger_system.warning(f"SL deviation: {abs(sl - current_price) / current_price * 100:.1f}%, TP deviation: {abs(tp - current_price) / current_price * 100:.1f}%")
                self.logger_system.warning("Deviation >10%, possibly API returned abnormal data")
                signal_data = self.create_fallback_signal(price_data)
        except (ValueError, TypeError) as e:
            self.logger_system.warning(f"Stop loss/take profit numeric conversion failed: {e}")
            self.logger_system.warning(f"Raw values - SL: {signal_data.get('stop_loss')}, TP: {signal_data.get('take_profit')}")
            self.logger_system.warning("Data type error, use backup signal")
            signal_data = self.create_fallback_signal(price_data)
    ...
    def confirm_order(self, order_id: str, expected_size: float, timeout: int = None) -> bool:
        ...
        while time.time() - start_time < timeout:
            try:
                ...
                # Safe handle CCXT filled field type conversion
                ...
                if status == 'closed':
                    if filled_size >= expected_size * self.config.min_fill_ratio:
                        self.logger_trading.info(f"Order {order_id} confirmation execution successful (filled: {filled_size:.4f}/{expected_size:.4f})")
                        return True
                    else:
                        self.logger_trading.warning(f"Order {order_id} partial fill: {filled_size:.4f} < {expected_size * self.config.min_fill_ratio:.4f}")
                        # Partial fill also considered success, but log warning
                        if filled_size > 0:
                            self.logger_trading.info(f"Order {order_id} partial fill successful, actual filled: {filled_size:.4f}")
                            return True
                        else:
                            self.logger_trading.error(f"Order {order_id} no fill")
                            return False
                elif status in ['canceled', 'rejected']:
                    self.logger_trading.error(f"Order {order_id} failed: {status}")
                    return False
                elif status == 'open':
                    # Check if partial fill and remaining small
                    if filled_size > 0 and remaining is not None:
                        ...
                        if remaining_size > 0 and remaining_size < expected_size * 0.05:  # Remaining <5%
                            try:
                                self.logger_trading.info(f"Order {order_id} partial fill {filled_size:.4f}, cancel remaining {remaining_size:.4f}")
                                ...
                                if final_filled >= expected_size * self.config.min_fill_ratio:
                                    self.logger_trading.info(f"Order {order_id} partial fill after cancel successful, final filled: {final_filled:.4f}")
                                    return True
                            except Exception as cancel_e:
                                self.logger_trading.warning(f"Cancel remaining order failed: {cancel_e}")
                    
                    # Order still executing, continue waiting
                    self.logger_trading.debug(f"Order {order_id} still executing, continue waiting...")
                    time.sleep(0.5)
            except Exception as e:
                self.logger_trading.exception(f"Order query failed {order_id}: {e}")
                time.sleep(0.5)
        
        # Timeout handling: Try cancel order and check final status
        try:
            self.logger_trading.warning(f"Order {order_id} timeout, attempting cancel...")
            exchange.cancel_order(order_id, self.config.symbol)
            time.sleep(1)
            final_order = self.safe_fetch_order(exchange, order_id, self.config.symbol)
            ...
            if final_filled >= expected_size * self.config.min_fill_ratio:
                self.logger_trading.info(f"Order {order_id} partial fill successful after timeout: {final_filled:.4f}")
                return True
            else:
                self.logger_trading.error(f"Order {order_id} timeout and insufficient fill: {final_filled:.4f}")
                return False
        except Exception as e:
            self.logger_trading.error(f"Order {order_id} timeout cancel failed: {e}")
            return False
    ...
    def calculate_dynamic_amount(self, price_data: Dict[str, Any], signal_data: Dict[str, Any], usd_balance: float, side: str) -> float:
        try:
            ...
            self.logger_trading.info(f"Isolated risk calculation: Balance={usd_balance:.2f}, Risk amount={risk_amount:.2f} ({risk_pct*100:.1f}%)")
            self.logger_trading.info(f"Leverage={leverage}x, MMR={mmr}, Liquidation distance={liq_distance:.2f} ({liq_factor*100:.2f}%)")
            self.logger_trading.info(f"Position size: {dynamic_amount:.6f} contracts")
            ...
            if position_value_usdc < min_position_value:
                self.logger_trading.warning(f"Position value {position_value_usdc:.2f} USDC below min {min_position_value} USDC")
                # Adjust to minimum USDC value
                dynamic_amount = min_position_value / entry_price
                self.logger_trading.info(f"Adjust position to min: {dynamic_amount:.6f} contracts (value {min_position_value} USDC)")
                ...
                if actual_risk_pct > risk_pct * 1.5:
                    self.logger_trading.error(f"Adjusted risk {actual_risk_pct*100:.1f}% too high, skip")
                    return 0
            ...
            try:
                ...
                if exchange_min_size and dynamic_amount < exchange_min_size:
                    self.logger_trading.warning(f"Adjust to exchange min quantity: {exchange_min_size}")
                    dynamic_amount = max(dynamic_amount, exchange_min_size)
            except Exception as e:
                self.logger_trading.debug(f"Unable to get exchange min order quantity: {e}")

            # Max margin usage
            notional = dynamic_amount * entry_price
            margin_required = notional / leverage
            if margin_required > usd_balance * self.config.max_margin_usage:
                self.logger_trading.warning("Exceeds max margin usage rate, reduce position")
            ...
            if atr_pct > 0.03:
                ...
                self.logger_trading.info(f"High volatility adjustment (ATR {atr_pct*100:.1f}%), position increase 10%")

            final_risk = dynamic_amount * liq_distance
            self.logger_trading.info(f"Final position: {dynamic_amount:.6f}, Expected risk: {final_risk:.2f} ({final_risk / usd_balance * 100:.1f}%)")

            return dynamic_amount
        except Exception as e:
            self.logger_trading.exception(f"Dynamic position calculation failed: {e}")
            return 0
    ...
    def calculate_rr(self, signal_data: Dict[str, Any], price_data: Dict[str, Any]) -> Tuple[float, float]:
        ...
        # Optimization 5: Inducement detection (simple Sweep check)
        inducement = False
        m15_df = price_data['multi_tf_data'].get('15m', pd.DataFrame())
        if not m15_df.empty and len(m15_df) > 1:
            prev_high = m15_df['high'].iloc[-2]
            current_high = m15_df['high'].iloc[-1]
            current_close = m15_df['close'].iloc[-1]
            if current_high > prev_high and current_close < prev_high:  # Fake breakout then close below
                inducement = True
                self.logger_trading.info("Inducement detected: Fake sweep confirmed")
    ...
    def _validate_and_prepare_trade(self, signal_data: Dict[str, Any], price_data: Dict[str, Any], activated_level: Optional[str] = None) -> Tuple[bool, float]:
        ...
        if signal_data['confidence'] == 'LOW' and not activated_level:  # Fixed: Check activated_level
            self.logger_trading.warning("Low confidence without activation, skipping")
            return False, 0
        
        # New: Multi-TF alignment check - Optimization 2
        structures = price_data['structures_summary']
        higher_tf_struct = structures.get(self.config.higher_tf_bias_tf, {})
        lower_tf_struct = structures.get(self.config.lower_tf_entry_tf, {})
        ...
        # New: Confirmation checks - Optimization 1 & 4
        # Volume
        ...
        # FVG stacking - Optimization 1
        key_levels = price_data['key_levels']
        fvg_bull = key_levels.get('fvg_bull_stack', 0)
        fvg_bear = key_levels.get('fvg_bear_stack', 0)
        ...
        # Initialize adjustment variable to avoid undefined error
        adjustment = 1.0
    ...
    def execute_trade(self, signal_data: Dict[str, Any], price_data: Dict[str, Any], activated_level: Optional[str] = None) -> None:
        ...
        trade_context = {
            "trade_id": trade_id,
            "symbol": self.config.symbol,
            "activated_level": activated_level or "N/A",  # Fix: Add or "N/A" avoid None
            "confidence": signal_data.get('confidence', 'UNKNOWN')
        }
        
        # FIXED: High 4 - Early skip for low conf no activation
        if signal_data['confidence'] == 'LOW' and activated_level is None:  # Fix: Check parameter if None
            self.logger_trading.info("Skip low confidence trade: ID=%s, no activation signal", trade_id)
            return
        
        if self.config.simulation_mode:
            self.logger_trading.info("Simulation trade: ID=%s, Signal=%s, Amount=%.4f", 
        ...
        if not valid:
            self.logger_trading.warning("HOLD signal, no trade needed: ID=%s", trade_id)
            return
        ...
        if position_value_usdc < self.config.min_amount_usdc:  # Changed from 0.001 BTC to 50 USDC
            self.logger_trading.error("Trade amount too small: ID=%s, Value=%.2f USDC < Min %.0f USDC", 
        ...
        self.logger_trading.info("Execute trade: ID=%s, Signal=%s, Amount=%.4f, SL=%.2f, TP=%.2f, R:R=%.1f:1, Activation=%s", 
                               trade_id, signal_data['signal'], dynamic_amount, 
                               signal_data.get('stop_loss', 0), signal_data.get('take_profit', 0),
                               rr_ratio, activated_level or "Scheduled")
        ...
        try:
            ...
            if order:
                ...
                self.logger_trading.info("Trade successful: ID=%s, Filled=%.4f @ %.2f, Leverage=%dx, Liquidation price=%.2f", 
                                       trade_id, dynamic_amount, entry_price, leverage, liq_price)
                
                # Update trade success stats
                if activated_level:
                    self._update_trade_stats(activated_level, success=True)
                    
        except Exception as e:
            self.logger_trading.error("Trade failed: ID=%s, Error=%s", trade_id, str(e))
            # Detailed error info with exception level, include context
            self.logger_trading.exception("Trade detailed error: ID=%s, symbol=%s, side=%s, amount=%.4f, activated=%s", 
                                        trade_id, self.config.symbol, side, dynamic_amount, activated_level)
            
            # Update trade failure stats
            if activated_level:
                self._update_trade_stats(activated_level, success=False)

    def _update_trade_stats(self, activated_level: str, success: bool):
        """Update trade statistics info"""
        with self.lock:
            if activated_level in self.level_activation_stats:
                if success:
                    ...
                    self.logger_trading.info(f"Key level {activated_level} trade success stats updated")
                else:
                    self.level_activation_stats[activated_level]['failed_trades'] += 1
                    self.total_failed_trades += 1
                    self.logger_trading.info(f"Key level {activated_level} trade failure stats updated")
                
                # Log updated win rate
                ...
                self.logger_trading.info(f"Key level {activated_level} current win rate: {win_rate:.1f}% ({stats['successful_trades']}/{total_trades})")
                # Optimization 6: Log win_rate sampling
                self.logger_system.debug(f"Win rate log: {activated_level} - {win_rate:.1f}%")

    def start_dynamic_sl_tp_monitor(self):
        """Start dynamic stop profit/loss monitoring thread"""
        if hasattr(self, 'sl_tp_monitor_thread') and self.sl_tp_monitor_thread and self.sl_tp_monitor_thread.is_alive():
            self.logger_trading.info("Dynamic stop profit/loss monitoring thread already running")
            return
        ...
        self.logger_trading.info("Dynamic stop profit/loss monitoring thread started")

    def dynamic_sl_tp_monitor_loop(self):
        """Dynamic stop profit/loss monitoring loop"""
        while True:
            try:
                ...
                self.logger_trading.exception(f"Dynamic stop profit/loss monitoring error: {e}")
                time.sleep(60)  # Wait longer on error

    def _update_dynamic_sl_tp(self, position: PositionInfo):
        """Update dynamic stop profit/loss"""
        try:
            ...
            # Calculate new Fibonacci stop profit/loss
            ...
            # 1. Profit protection: When profit >2%, move stop loss to breakeven
            if pnl_pct > 0.02:
                ...
                if protective_sl and self._should_update_stop_loss(position, protective_sl, side):
                    self.logger_trading.info(f"Profit protection: Move stop loss to {protective_sl:.2f} (current profit: {pnl_pct*100:.2f}%)")
                    adjustment_made = True
            
            # 2. Trend tracking: Adjust stop profit/loss based on technical indicators
            ...
            if trend_adjustment:
                ...
                if self._should_update_stop_loss(position, trend_sl, side):
                    self.logger_trading.info(f"Trend tracking: Adjust stop loss to {trend_sl:.2f}")
                    adjustment_made = True
                if self._should_update_take_profit(position, trend_tp, side):
                    self.logger_trading.info(f"Trend tracking: Adjust take profit to {trend_tp:.2f}")
                    adjustment_made = True
            
            # 3. Volatility adjustment: Adjust stop loss distance based on ATR
            ...
            if volatility_adjustment and self._should_update_stop_loss(position, volatility_adjustment, side):
                self.logger_trading.info(f"Volatility adjustment: Adjust stop loss to {volatility_adjustment:.2f}")
                adjustment_made = True
            
            # 4. Fibonacci optimization: Recalculate Fibonacci levels periodically
            if new_sl and new_tp:
                fib_improvement = self._evaluate_fibonacci_improvement(position, new_sl, new_tp, side)
                if fib_improvement:
                    self.logger_trading.info(f"Fibonacci optimization: Stop loss {new_sl:.2f}, Take profit {new_tp:.2f}")
                    adjustment_made = True
            
            if adjustment_made:
                self.logger_trading.info(f"Dynamic stop profit/loss updated - Current price: {current_price:.2f}, Profit: {pnl_pct*100:.2f}%")
                
        except Exception as e:
            self.logger_trading.exception(f"Update dynamic stop profit/loss failed: {e}")

    def _calculate_protective_stop_loss(self, entry_price: float, current_price: float, 
                                      side: str, pnl_pct: float) -> Optional[float]:
        """Calculate protective stop loss price"""
        try:
            if side == 'long':
                # Long: Move stop loss to breakeven or higher
                if pnl_pct > 0.05:  # Profit >5%
                    return entry_price * 1.02  # Protect 2% profit
                elif pnl_pct > 0.02:  # Profit >2%
                    return entry_price * 1.005  # Move to breakeven above
            else:
                # Short: Move stop loss to breakeven or lower
                if pnl_pct > 0.05:  # Profit >5%
                    return entry_price * 0.98  # Protect 2% profit
                elif pnl_pct > 0.02:  # Profit >2%
                    return entry_price * 0.995  # Move to breakeven below
            return None
        except Exception as e:
            self.logger_trading.error(f"Calculate protective stop loss failed: {e}")
            return None

    def _calculate_trend_based_adjustment(self, df: pd.DataFrame, current_price: float, 
                                        side: str) -> Optional[Tuple[float, float]]:
        """Trend-based stop profit/loss adjustment"""
        try:
            if len(df) < 20:
                return None
            ...
            self.logger_trading.error(f"Calculate trend adjustment failed: {e}")
            return None
    ...
    def _calculate_volatility_based_adjustment(self, df: pd.DataFrame, current_price: float, 
        ...
        try:
            ...
            if atr_pct > 0.03:  # High volatility
                multiplier = 2.5
            elif atr_pct > 0.02:  # Medium volatility
                multiplier = 2.0
            else:  # Low volatility
                multiplier = 1.5
            ...
            self.logger_trading.error(f"Calculate volatility adjustment failed: {e}")
            return None

    def _should_update_stop_loss(self, position: PositionInfo, new_sl: float, side: str) -> bool:
        """Judge if should update stop loss"""
        try:
            current_sl = position.get('liquidation_price', 0)  # Use liquidation price as current stop loss
            
            if side == 'long':
                # Long: New stop loss should be higher (safer)
                return new_sl > current_sl
            else:
                # Short: New stop loss should be lower (safer)
                return new_sl < current_sl
        except Exception as e:
            self.logger_trading.error(f"Judge stop loss update failed: {e}")
            return False

    def _should_update_take_profit(self, position: PositionInfo, new_tp: float, side: str) -> bool:
        """Judge if should update take profit"""
        try:
            # Here can add more complex logic to judge if update take profit
            # Currently simply return True, always try to optimize take profit
            return True
        except Exception as e:
            self.logger_trading.error(f"Judge take profit update failed: {e}")
            return False

    def _evaluate_fibonacci_improvement(self, position: PositionInfo, new_sl: float, 
                                      new_tp: float, side: str) -> bool:
        """Evaluate if Fibonacci adjustment improves"""
        try:
            entry_price = position['entry_price']
            ...
            # Only adjust if risk/reward ratio improves and >1.5
            return new_rr > 1.5
            
        except Exception as e:
            self.logger_trading.error(f"Evaluate Fibonacci improvement failed: {e}")
            return False

    # New: Intraday momentum filter - Enhanced with confirmations
    def intraday_momentum_filter(self, price_data: Dict[str, Any]) -> bool:
        """Momentum filter with price, volume, and new confirmations"""
        try:
            m15_df = price_data['multi_tf_data'].get('15m')
            if m15_df is None or len(m15_df) < 20:
            ...
            if vol_ratio_15m < self.config.volume_confirmation_threshold:
                self.logger_trading.info("Momentum filter failed: Volume < 1.2x MA")
                return False
            
            # Price > EMA12 check for bullish bias (adjust for signal if available)
            ...
            if fvg_bull_stack < 1 and fvg_bear_stack < 1:  # At least one stack
                self.logger_trading.info("Momentum filter failed: No FVG stacking")
                return False
            
            self.logger_trading.info("Intraday momentum filter passed (volume, EMA, patterns, FVG)")
            return True
        except Exception as e:
    ...
    def _fetch_and_update_data(self, activated_level: Optional[str] = None) -> Optional[Dict[str, Any]]:
        if self.config.simulation_mode:
            # In simulation mode, directly get data, no need for real-time price check
            price_data = self.get_multi_timeframe_data()
            if not price_data:
                return None
            current_price = price_data['current_price']
            self.logger_system.info(f"Simulation price: ${current_price:,.2f}")
        else:
            ticker = self.safe_fetch_ticker(exchange, self.config.symbol)
            current_price = round(ticker['last'], 2) if ticker else 67000.0
            self.logger_system.info(f"Real-time price: ${current_price:,.2f}")
        ...
        if not activated_level:
            with self.lock:
                if self.key_levels_cache:
                    ...
                    if not is_activated:
                        self.logger_system.info("Price not close to key level, skip analysis")
                        return None
        ...
        try:
            ...
            if latest_ohlcv:
                ...
                new_row = new_row.set_index('timestamp')  # Set timestamp as index, consistent with df_primary
    ...
    def trading_bot(self, activated_level: Optional[str] = None, is_scheduled: bool = False):
        """Main trade logic execution method - Optimization 3: Add Kill Zone filter"""
        ...
        try:
            ...
            # Optimization 3: Kill Zone time filter (UTC 8-16 for London/NY)
            ...
            if not signal_data:
                self.logger_system.error("Unable to get trade signal, skip this trade")
                return
